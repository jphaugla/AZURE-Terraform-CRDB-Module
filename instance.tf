...
if [[ '${var.run_init}' = 'yes' && ${count.index + 1} -eq ${var.crdb_nodes} && ${var.create_admin_user} = 'yes' ]]; then echo "Creating admin user ${var.admin_user_name}" && su ${local.admin_username} -lc 'cockroach sql --execute "create user ${var.admin_user_name} with login password ${var.admin_user_password}; grant admin to ${var.admin_user_name} with admin option"'; fi
