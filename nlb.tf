...
resource "azurerm_network_interface" "load_balancer_ni" {
  ...
  ip_configuration {
    ...
    public_ip_address_id       = azurerm_public_ip.network-interface-ip.id

  }
}

resource "azurerm_lb_rule" "admin_rule" {
  loadbalancer_id = azurerm_lb.azure_load_balancer.id
  name                 = "${var.owner}-${var.resource_name}-crdb-admin-rule"
  protocol                 = "Tcp"
  ...
  backend_address_pool_ids       = [azurerm_lb_backend_address_pool.crdb_pool.id]
  probe_id                = azurerm_lb_probe.crdb_probe.id
}
 
resource "azurerm_lb_rule" "crdb_rule" {
  loadbalancer_id = azurerm_lb.azure_load_balancer.id
  name                 = "${var.owner}-${var.resource_name}-crdb-rule"
  protocol                 = "Tcp"
...
resource "azurerm_lb_probe" "crdb_probe" {
  name                 = "${var.owner}-${var.resource_name}-crdb-probe"
  loadbalancer_id       = azurerm_lb.azure_load_balancer.id
  port                 = 8080
...
resource "azurerm_lb_backend_address_pool" "crdb_pool" {
  name = "${var.owner}-${var.resource_name}-crdb-backend-pool"
  loadbalancer_id = azurerm_lb.azure_load_balancer.id
}


resource "azurerm_network_interface_backend_address_pool_association" "backendcrdb" {
  ...
  backend_address_pool_id = azurerm_lb_backend_address_pool.crdb_pool.id
}

resource "azurerm_lb" "azure_load_balancer" {
  name                 = "${var.owner}-${var.resource_name}-load-balancer"
  location             = var.virtual_network_location
  resource_group_name  = local.resource_group_name
  sku                  = "Standard"  # Ensure Standard SKU for backend address pool
  frontend_ip_configuration {
    ...
    private_ip_address   = azurerm_network_interface.load_balancer_ni[0].private_ip_address
  }
}
