    ...
    content = templatefile("${path.module}/${var.inventory_template_file}",
        {
            ...
            load_balancer_public_ip = length(azurerm_public_ip.load_balancer-ip.id) > 0 ? azurerm_public_ip.load_balancer-ip.id : null
            load_balancer_private_ip = length(azurerm_network_interface.load_balancer_ni) > 0 ? azurerm_network_interface.load_balancer_ni[0].private_ip_address : null
